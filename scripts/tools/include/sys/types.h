 ...
 * requires 17 bits to express, so size_t must be a int type.  The type 
...
#ifndef _MNX_TIME_T
#define _MNX_TIME_T
typedef int mnx_time_t;    /* time in sec since 1 Jan 1970 0000 GMT */
#endif

#ifndef _MNX_CLOCK_T
#define _MNX_CLOCK_T
typedef int MNX(clock_t);   /* unit for system accounting */
#endif

#ifndef _MNX_SIGSET_T
#define _MNX_SIGSET_T
typedef unsigned int MNX(sigset_t);
#endif

/* Open Group Base Specifications Issue 6 (not complete) */
typedef int MNX(useconds_t);           /* Time in microseconds */

/* Types used in disk, inode, etc. data structures. */
typedef short          MNX(dev_t);      /* holds (major|minor) device pair */
typedef char           MNX(gid_t);      /* group id */
typedef unsigned int  MNX(ino_t);      /* i-node number (V3 filesystem) */
typedef unsigned short MNX(mode_t);     /* file type and permissions bits */
typedef short          MNX(nlink_t);    /* number of links to a file */
typedef int	       MNX(off_t);	   /* offset within a file */
typedef int            MNX(pid_t);      /* process id (must be signed) */
typedef short          MNX(uid_t);      /* user id */
typedef unsigned int  MNX(zone_t);     /* zone number */
typedef unsigned int  MNX(block_t);    /* block number */
typedef unsigned int  MNX(bit_t);      /* bit number in a bit map */
...
typedef unsigned int  MNX(u32_t);   /* 32 bit type */

typedef char            MNX(i8_t);      /* 8 bit signed type */
typedef short          MNX(i16_t);      /* 16 bit signed type */
typedef int           MNX(i32_t);      /* 32 bit signed type */

#if __WORDISIZE == 64
typedef unsigned long int MNX(u64_t);
...
typedef unsigned int MNX(U32_t);
typedef int            MNX(I8_t);
typedef int           MNX(I16_t);
typedef int          MNX(I32_t);
...
typedef unsigned int  MNX(Ino_t);
