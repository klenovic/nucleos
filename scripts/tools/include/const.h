...
#define SUPER_USER (MNX(uid_t)) 0	/* uid_t of superuser */
...
/* Some limits. */
#define MAX_INODE_NR ((MNX(ino_t)) 037777777777)	/* largest inode number */
#define MAX_FILE_POS ((MNX(off_t)) 0x7FFFFFFF)	/* largest legal file offset */

#define MAX_SYM_LOOPS	8	/* how many symbolic links are recursed */

#define NO_BLOCK              ((MNX(block_t)) 0)	/* absence of a block number */
#define NO_ENTRY                ((MNX(ino_t)) 0)	/* absence of a dir entry */
#define NO_ZONE                ((MNX(zone_t)) 0)	/* absence of a zone number */
#define NO_DEV                  ((MNX(dev_t)) 0)	/* absence of a device numb */
