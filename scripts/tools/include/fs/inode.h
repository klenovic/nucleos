...
extern struct MNX(inode) {
  ...
  MNX(gid_t) i_gid;      /* group number */
  MNX(off_t) i_size;     /* current file size in bytes */
  MNX(time_t) i_atime;   /* time of last access (V2 only) */
  MNX(time_t) i_mtime;   /* when was file data last changed */
  MNX(time_t) i_ctime;   /* when was inode itself changed (V2 only)*/
  MNX(zone_t) i_zone[V2_NR_TZONES]; /* zone numbers for direct, ind, and dbl ind */

  /* The following items are not present on the disk. */
  MNX(dev_t) i_dev; /* which device is the inode on */
  MNX(ino_t) i_num; /* inode number on its (minor) device */
  int i_count;      /* # times inode used; 0 means slot is free */
  int i_ndzones;    /* # direct zones (Vx_NR_DZONES) */
  int i_nindirs;    /* # indirect zones per indirect block */
  struct MNX(super_block) *i_sp; /* pointer to super block for inode's device */
  char i_dirt;   /* CLEAN or DIRTY */
  char i_pipe;   /* set to I_PIPE if pipe */
  MNX(bit_t) i_zsearch;		/* where to start search for new zones */
  ...
  LIST_ENTRY(MNX(inode)) i_hash;     /* hash list */
  TAILQ_ENTRY(MNX(inode)) i_unused;  /* free and unused list */
