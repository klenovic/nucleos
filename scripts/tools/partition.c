 ...
 */
char *device;
struct MNX(part_entry) primary, table[2 * NR_PARTITIONS + 1];
int npart;
...
void find_exist(struct MNX(part_entry) *exist, int sysind, int nr)
{
	int f;
	MNX(u16_t) signature;
	struct MNX(part_entry) oldtable[NR_PARTITIONS];
	int n, i;
	MNX(u32_t) minlow, curlow;
	struct MNX(part_entry) *cur;
...
{
	int f;
	MNX(u16_t) signature= 0xAA55;
	struct MNX(part_entry) newtable[NR_PARTITIONS];
	int i;
...
void show_part(struct MNX(part_entry) *p)
{
	static int banner= 0;
	int n;
	...
	printf("%3d ", (n-1) / 2); /* partition */
	show_chs(p->lowsec); /* first */
	show_chs(p->lowsec + p->size - 1); /* last */
	printf("  %8lu  %8lu  %7lu\n", p->lowsec, p->size, p->size / 2); /* Base, Size, Kb */
}
...
//	struct MNX(partition) geometry;
	...
	if (ioctl(fd, HDIO_GETGEO, &geometry) < 0)
	{
		...
		if (fstat(fd, &sb) < 0)
			fatal(device);
		geometry.start= (MNX(u64_t))0;
		devsize64 = (MNX(u64_t))sb.st_size;
		geometry.sectors= 32;
		geometry.heads= 64;
	...
	if (ioctl(fd, BLKGETSIZE64, &devsize64) != 0) {
		fprintf(stderr,"partition: can't device size.\n");
		devsize64 = (MNX(u64_t))sb.st_size;
		return;
	}
	...
	primary.lowsec= (unsigned long)(geometry.start/SECTOR_SIZE);
	primary.size= (unsigned long)(devsize64/SECTOR_SIZE);
...
void boundary(struct MNX(part_entry) *pe, int exp)
