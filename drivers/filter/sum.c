 ...
 int count)
{
	 ...
	 * make_group_sum(). Return 0 if all checksums check out, or RET_REDO
	 * upon failure.
	 */
...
static int check_sum(sector_t current_sector, size_t bytes_left)
{
	 ...
	 * Return 0 if all checksums are okay, or RET_REDO upon failure.
	 */
...
static int check_write(u64_t pos, size_t size)
{
	 ...
	 * enabled, and check the result against the original. Return 0 on
