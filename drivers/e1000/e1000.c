...
static e1000_t * e1000_port(int port);
static uint32_t e1000_reg_read(e1000_t *e, uint32_t reg);
static void e1000_reg_write(e1000_t *e, uint32_t reg,
					  uint32_t value);
static void e1000_reg_set(e1000_t *e, uint32_t reg,
					  uint32_t value);
static void e1000_reg_unset(e1000_t *e, uint32_t reg,
					   uint32_t value);
...
int main(int argc, char *argv[])
{
    ...
    if ((r = ds_retrieve_u32("inet", &tasknr)) == 0)
    {
	...
	{
	    panic("e1000", "kipc_module_call type KIPC_RECEIVE  failed", r);
	}
...
e1000_t *e;
{
     ...
     */
    if ((r = sys_irqsetpolicy(e->irq, 0, &e->irq_hook)) != 0)
    {
        panic(e->name, "sys_irqsetpolicy failed", r);
    }
    if ((r = sys_irqenable(&e->irq_hook)) != 0)
    {
 ...
 *				e1000_report_link			     *
 *===========================================================================*/
static void e1000_report_link(e)
e1000_t *e;
{
 ...
 *				reply					     *
 *===========================================================================*/
static void reply(e, err, may_block)
e1000_t *e;
int err;
int may_block;
{
