...
static int w_controller = -1;
static int w_major = -1;

static int win_tasknr;			/* my task number */
static int w_command;			/* current command in execution */
static u8_t w_byteval;			/* used for SYS_IRQCTL */
static int w_drive;			/* selected drive */
static int w_controller;		/* selected controller */
...
#if ENABLE_ATAPI
static int atapi_sendpacket(u8_t *packet, unsigned cnt);
static int atapi_intr_wait(void);
...
static int at_voutb(int line, pvb_pair_t *, int n);
#define sys_vinb(in, n) at_vinb(__LINE__, (in), (n))
static int at_vinb(int line, pvb_pair_t *, int n);
...
#define id_longword(n)	(((u32_t) id_byte(n)[0] <<  0) \
  ...
  if (r == 0 && w_waitfor(STATUS_DRQ, STATUS_DRQ) &&
	...
	if ((w & ID_CAP_LBA) && size > 512L*1024*2) {
		...
		if (dma_base)
		{
			if (sys_inb(dma_base + DMA_STATUS, (u32_t*)&dma_status) != 0)
			{
				panic(w_name(),
  ...
  if (cmd.command = ATAPI_IDENTIFY,
	com_simple(&cmd) == 0 && w_waitfor(STATUS_DRQ, STATUS_DRQ) &&
	!(wn->w_status & (STATUS_ERR|STATUS_WF))) {
	/* An ATAPI device. */
	wn->state |= ATAPI;
  ...
  if (wn->irq == NO_IRQ) {
	  ...
	  if ((s=sys_irqsetpolicy(wn->irq, IRQ_REENABLE, &wn->irq_hook_id)) != 0) 
	  	panic(w_name(), "couldn't set IRQ policy", s);
	  if ((s=sys_irqenable(&wn->irq_hook_id)) != 0)
...
/* Routine to initialize the drive after boot or when a reset is needed. */
  ...
  if (!(wn->state & ATAPI)) {
	...
	if (com_simple(&cmd) != 0) return(ERR);
 ...
 *				do_transfer				     *
 *===========================================================================*/
static int do_transfer(struct wini *wn, unsigned int precomp,
...
int safe;			/* iov contains addresses (0) or grants? */
{
  ...
  while (nr_req > 0) {
	...
	if (cmp64(position, dv_size) >= 0) return 0;		/* At EOF */
	...
	if (!(wn->state & INITIALIZED) && w_specify() != 0) return(-EIO);
	...
	if (opcode == DEV_SCATTER_S) {
		 ...
		 */
		if (sys_inb((wn->base_ctl+REG_CTL_ALTSTAT), (u32_t*)&w_status) != 0)
...
/* Output the command block to the winchester controller and return status */
  ...
  /* Select drive. */
  if ((s=sys_outb(base_cmd + REG_LDH, cmd->ldh)) != 0)
...
/* A simple controller command, only one interrupt and no data-out phase. */
  ...
  if ((r = com_out(cmd)) == 0) r = at_intr_wait();
  w_command = CMD_IDLE;
  return(r);
}
 ...
 */
  ...
  /* Strobe reset bit */
  if ((s=sys_outb(wn->base_ctl + REG_CTL, CTL_RESET)) != 0)
  	panic(w_name(),"Couldn't strobe reset bit",s);
  tickdelay(DELAY_TICKS);
  if ((s=sys_outb(wn->base_ctl + REG_CTL, 0)) != 0)
  	panic(w_name(),"Couldn't strobe reset bit",s);
  tickdelay(DELAY_TICKS);
...
/* Wait for an interrupt, study the status bits and return error/success. */
  ...
  if ((w_wn->w_status & (STATUS_BSY | STATUS_WF | STATUS_ERR)) == 0) {
	r = 0;
  	...
  	if ((s=sys_inb(w_wn->base_cmd + REG_ERROR, (u32_t*)&inbval)) != 0)
 ...
 */
  ...
  getuptime(&t0);
  do {
	if ((s=sys_inb(w_wn->base_cmd + REG_STATUS, (u32_t*)&w_status)) != 0)
		panic(w_name(),"Couldn't read register",s);
	w_wn->w_status= w_status;
 ...
 */
  ...
  getuptime(&t0);
  do {
	if ((s=sys_inb(w_wn->base_dma + DMA_STATUS, (u32_t*)&w_status)) != 0)
		panic(w_name(),"Couldn't read register",s);
...
int safe;			/* use safecopies? */
{
  ...
  while (nr_req > 0 && !fresh) {
	...
	if ((s=sys_insw(wn->base_cmd + REG_DATA, SELF, tmp_buf, chunk)) != 0)
		panic(w_name(),"Call to sys_insw() failed", s);
			before -= chunk;
			count -= chunk;
		}
 ...
 *				atapi_sendpacket			     *
 *===========================================================================*/
static int atapi_sendpacket(packet, cnt)
u8_t *packet;
unsigned cnt;
{
/* Send an Atapi Packet Command */
  ...
  if ((s=sys_outb(wn->base_cmd + REG_DRIVE, wn->ldhpref)) != 0)
...
static int at_voutb(int line, pvb_pair_t *pvb, int n)
{
  int s, i;
...
static int at_vinb(int line, pvb_pair_t *pvb, int n)
{
  int s, i;
...
static int at_out(int line, u32_t port, u32_t value,
...
static int at_in(int line, u32_t port, u32_t *value,
	char *typename, int type)
{
	int s;
