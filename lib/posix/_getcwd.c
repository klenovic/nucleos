...
char *getcwd(char *path, size_t size)
{
	...
	if (path == 0 || size <= 1) { errno= EINVAL; return 0; }
	...
	*--p = 0;
	...
	while (1) {
		dotdot= "..";
		if (stat(dotdot, &above) < 0) { recover(p); return 0; }

		if (above.st_dev == current.st_dev
					&& above.st_ino == current.st_ino)
			break;	/* Root dir found */

		if ((d= opendir(dotdot)) == 0) { recover(p); return 0; }
		...
		if (chdir(dotdot) < 0) { recover(p); return 0; }
		p= up;
