...
/* Main program of any device driver task. */
  ...
  while (TRUE) {
	...
	/* Wait for a request to read or write a disk block. */
		if ((s=kipc_module_call(KIPC_RECEIVE, 0, ENDPT_ANY, &mess)) != 0)
 ...
 */
  ...
  if (0 != sys_safecopyfrom(mp->m_source, (vir_bytes) mp->IO_GRANT, 
  ...
  if (0 != sys_safecopyto(mp->m_source, (vir_bytes) mp->IO_GRANT, 
		0, (vir_bytes) iovec, iovec_size, D)) {
