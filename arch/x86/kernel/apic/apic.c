...
int reboot_type;
int ioapic_enabled;
u32_t ioapic_id_mask[8], lapic_id_mask[8];
u32_t lapic_addr_vaddr;
u32_t lapic_addr;
u32_t lapic_eoi_addr;
u32_t lapic_taskpri_addr;
int bsp_lapic_id;

static volatile int probe_ticks;
...
static u32_t pci_config_intr_data;
static u32_t ioapic_extint_assigned = 0;
static int lapic_extint_assigned = 0;

static int calib_clk_handler(irq_hook_t * hook)
{
	u32_t tcrt;
	u64_t tsc;
...
void apic_calibrate_clocks(void)
{
	...
	BOOT_VERBOSE(printk("Calibrating clock\n"));
	/*
	...
	BOOT_VERBOSE(printk("APIC bus freq %lu MHz\n",
				lapic_bus_freq[cpuid()] / 1000000));
	cpu_freq = div64u(tsc_delta, PROBE_TICKS - 1) * system_hz;
	BOOT_VERBOSE(printk("CPU %d freq %lu MHz\n", cpuid(),
				cpu_freq / 1000000));
}

static void lapic_set_timer_one_shot(u32_t value)
{
...
static void lapic_enable_no_lints(void)
{
	u32_t val;
...
static int lapic_enable_in_msr(void)
{
	...
	if (phys2vir(addr) != (lapic_addr >> 12)) {
		if (msr.hi & 0xf) {
			printk("ERROR : APIC address needs more then 32 bits\n");
			return 0;
		}
...
static void apic_spurios_intr(void)
{
...
static lapic_set_dummy_handlers(void)
{
	char * handler;
	int vect = 32;
...
void apic_idt_init(int reset)
{
...
#ifdef CONFIG_DEBUG_X86_LOCAL_APIC
	if (cpu_is_bsp(cpuid()))
		printk("APIC debugging is enabled\n");
	...
	if (cpu_is_bsp(cpuid())) {
		...
		BOOT_VERBOSE(printk("Initiating BSP timer handler\n"));
	} else {
		local_timer_intr_handler = (vir_bytes) lapic_ap_timer_int_handler;
		BOOT_VERBOSE(printk("Initiating AP timer handler\n"));
	}
