...
struct gatedesc_s {
	u16 offset_low;
	u16 selector;
	u8 pad;                     /* |000|XXXXX| ig & trpg, |XXXXXXXX| task g */
	u8 p_dpl_type;              /* |P|DL|0|TYPE| */
	u16 offset_high;
};
 ...
 */
	...
	segdp = &gdt[seg >> 3];
	base = ((u32)segdp->base_low << 0) |
	       ((u32) segdp->base_middle << 16) |
	       ((u32) segdp->base_high << 24);

	return base;
}

void phys2seg(u16 *seg, vir_bytes *off, phys_bytes phys)
{
...
void init_dataseg(register struct segdesc_s *segdp, phys_bytes base, vir_bytes size, int privilege)
{
 ...
 */
	...
	if(kinfo.data_base % CLICK_SIZE)
		kernel_panic("kinfo.data_base not aligned", NO_NUM);
	...
	* (u32 *) dtp->base = vir2phys(idt);
	...
	for (rp = BEG_PROC_ADDR, ldt_index = FIRST_LDT_INDEX; rp < END_PROC_ADDR; ++rp, ldt_index++) {
		init_dataseg(&gdt[ldt_index], vir2phys(rp->p_seg.p_ldt), sizeof(rp->p_seg.p_ldt), INTR_PRIVILEGE);
...
#define CHECK(s1, s2) if(s1 != s2) {		\
	...
	if (fail) {
		printk("%d/%d checks failed\n", fail, checked);
		kernel_panic("wrong", fail);
	}
}

void printseg(char *banner, int iscs, struct proc *pr, u32 selector)
{
	u32 base, limit, index, dpl;
	struct segdesc_s *desc;

	if(banner) { printk("%s", banner); }

	index = selector >> 3;

	printk("RPL %d, ind %d of ",
		(selector & RPL_MASK), index);

	if(selector & TI) {
		printk("LDT");
		if(index < 0 || index >= LDT_SIZE) {
			printk("invalid index in ldt\n");
			return;
		}
		if(!pr) {
			printk("local selector but unknown process\n");
			return;
		}
	...
	limit = desc->limit_low |
		(((u32) desc->granularity & LIMIT_HIGH) << GRANULARITY_SHIFT);
	...
	base = desc->base_low | 
		((u32) desc->base_middle << BASE_MIDDLE_SHIFT) |
		((u32) desc->base_high << BASE_HIGH_SHIFT);

	printk(" -> base 0x%08lx size 0x%08lx ", base, limit+1);
	...
	if(desc->granularity & 0x20) {	/* reserved */
		kernel_panic("granularity reserved field set", NO_NUM);
	}
	...
	if(desc->access & EXECUTABLE) {
		printk("   exec ");
		if(desc->access & CONFORMING) printk("conforming ");
		if(!(desc->access & READABLE)) printk("non-readable ");
	} else {
		printk("nonexec ");
		if(desc->access & EXPAND_DOWN) printk("non-expand-down ");
		if(!(desc->access & WRITEABLE)) printk("non-writable ");
	}
	...
	dpl = ((u32) desc->access & DPL) >> DPL_SHIFT;
...
int prot_set_kern_seg_limit(vir_bytes limit)
{
	...
	if(limit <= kinfo.data_base) {
		printk("prot_set_kern_seg_limit: limit bogus\n");
		return -EINVAL;
	}
