...
int intr_init(int mine, int auto_eoi)
{
	 ...
	 * to be programmed for Nucleos, or to be reset to what the BIOS expects.
	 */
	if (!intr_disabled())
		intr_disable();
	...
	*/
	...
	outb( INT_CTLMASK, mine == INTS_NUCLEOS ? IRQ0_VECTOR : BIOS_IRQ0_VEC);
                                                  /* ICW2 for master */
	...
	outb( INT2_CTLMASK, mine == INTS_NUCLEOS ? IRQ8_VECTOR : BIOS_IRQ8_VEC);
                                                  /* ICW2 for slave */
	...
	/* Copy the BIOS vectors from the BIOS to the Nucleos location, so we
