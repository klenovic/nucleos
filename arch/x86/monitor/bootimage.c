...
/* Get a Minix image into core, patch it up and execute. */
{
  ...
  /* Minix. */
#ifdef CONFIG_MEMORY_MODEL_FLAT 
...
int load_initrd(char* initrd, unsigned long loadaddr)
{
  ...
  if((nvsec = select_initrd(initrd)) > 0) {
    ...
    if (loadaddr + initrd_size > MAX_RAWCOPY_ADDR) {
      printf("error: initrd crosses 0x%x address (load:0x%x size:%d)\n", MAX_RAWCOPY_ADDR, loadaddr, initrd_size);
      return -1;
    }
 ...
 */
{
  ...
  if ((image = select_image(b_value("image"))) == nil)
    return;
