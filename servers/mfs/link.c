...
static int remove_dir_o(struct inode *rldirp, struct inode *rip, char dir_name[NAME_MAX]);
static int remove_dir_nocheck(struct inode *rldirp, struct inode *rip, char dir_name[NAME_MAX]);
static int unlink_file_o(struct inode *dirp, struct inode *rip, char file_name[NAME_MAX]);
static int unlink_file_nocheck(struct inode *dirp, struct inode *rip, char file_name[NAME_MAX]);
 ...
 */
  ...
  if (fs_m_in.m_type == REQ_UNLINK_O) {
	...
	if (r == 0) r = unlink_file_o(rldirp, rip, string);
  ...
  put_inode(rip);
  put_inode(rldirp);
  if (r != 0) printf("fs_unlink_o: returning %d\n", r);
  return(r);
}
 ...
 */
  ...
  if (fs_m_in.m_type == REQ_UNLINK_S) {
	...
	/* Actually try to unlink the file; fails if parent is mode 0 etc. */
	if (r == 0) r = unlink_file_nocheck(rldirp, rip, string);
  } 
  else {
 ...
 *				remove_dir_o				     *
 *===========================================================================*/
static int remove_dir_o(rldirp, rip, dir_name)
...
char dir_name[NAME_MAX];		/* name of directory to be removed */
{
  ...
  if ((r = search_dir(rip, "", (ino_t *) 0, IS_EMPTY)) != 0) return r;

  if (strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)return(-EINVAL);
  if (rip->i_num == ROOT_INODE) return(-EBUSY); /* can't remove 'root' */
 
  /* Actually try to unlink the file; fails if parent is mode 0 etc. */
  if ((r = unlink_file_o(rldirp, rip, dir_name)) != 0) return r;
 ...
 *				remove_dir_nocheck			     *
 *===========================================================================*/
static int remove_dir_nocheck(rldirp, rip, dir_name)
struct inode *rldirp;		 	/* parent directory */
struct inode *rip;			/* directory to be removed */
char dir_name[NAME_MAX];		/* name of directory to be removed */
{
  ...
  /* search_dir checks that rip is a directory too. */
  if ((r = search_dir_nocheck(rip, "", (ino_t *) 0, IS_EMPTY)) != 0) return r;

  if (strcmp(dir_name, ".") == 0 || strcmp(dir_name, "..") == 0)return(-EINVAL);
  if (rip->i_num == ROOT_INODE) return(-EBUSY); /* can't remove 'root' */
 
  /* Actually try to unlink the file; fails if parent is mode 0 etc. */
  if ((r = unlink_file_nocheck(rldirp, rip, dir_name)) != 0) return r;
 ...
 *				unlink_file_o				     *
 *===========================================================================*/
static int unlink_file_o(dirp, rip, file_name)
...
/* Unlink 'file_name'; rip must be the inode of 'file_name' or NIL_INODE. */
  ...
  if (rip == NIL_INODE) {
	...
	if (err_code != 0 || rip == NIL_INODE) return(err_code);
  } else {
 ...
 *				unlink_file_nocheck			     *
 *===========================================================================*/
static int unlink_file_nocheck(dirp, rip, file_name)
struct inode *dirp;		/* parent directory of file */
struct inode *rip;		/* inode of file, may be NIL_INODE too. */
char file_name[NAME_MAX];	/* name of file to be removed */
{
...
/* Perform the rename(name1, name2) system call. */
  ...
  if (r == 0) {
	...
	if ((r1 = forbidden(old_dirp, W_BIT | X_BIT)) != 0 ||
	    (r1 = forbidden(new_dirp, W_BIT | X_BIT)) != 0) {
		r = r1;
	}
	...
	numb = old_ip->i_num;		/* inode number of old file */
  	...
  	if (same_pdir) {
		r = search_dir(old_dirp, old_name, (ino_t *) 0, DELETE);
						/* shouldn't go wrong. */
		if (r == 0) (void) search_dir(old_dirp, new_name, &numb, ENTER);
	} else {
  ...
  if (r == 0 && odir && !same_pdir) {
	...
	if (search_dir(old_ip, dot2, &numb, ENTER) == 0) {
		/* New link created. */
		new_dirp->i_nlinks++;
...
printf("fs_rename_s: next_new_superdirp: %d on 0x%x\n",
	...
	numb = old_ip->i_num;		/* inode number of old file */
  	...
  	if (same_pdir) {
		r = search_dir_nocheck(old_dirp, old_name, (ino_t *) 0, DELETE);
						/* shouldn't go wrong. */
		if (r == 0) (void) search_dir_nocheck(old_dirp, new_name,
			&numb, ENTER);
	} else {
  ...
  if (r == 0 && odir && !same_pdir) {
	...
	if (search_dir_nocheck(old_ip, dot2, &numb, ENTER) == 0) {
		/* New link created. */
		new_dirp->i_nlinks++;
		new_dirp->i_dirt = DIRTY;
	}
 ...
 *				freesp_inode				     *
 *===========================================================================*/
int freesp_inode(rip, start, end)
