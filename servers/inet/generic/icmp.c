...
static acc_t *icmp_getdata(int port, size_t offset, size_t count, int for_ioctl);
static int icmp_putdata(int port, size_t offset, acc_t *data, int for_ioctl);
...
static void icmp_echo_request(icmp_port_t *icmp_port, acc_t *ip_pack, int ip_hdr_len,
			      ip_hdr_t *ip_hdr, acc_t *icmp_pack, int icmp_len,
			      icmp_hdr_t *icmp_hdr);
static void icmp_dst_unreach(icmp_port_t *icmp_port, acc_t *ip_pack, int ip_hdr_len,
			     ip_hdr_t *ip_hdr, acc_t *icmp_pack, int icmp_len,
			     icmp_hdr_t *icmp_hdr);
static void icmp_time_exceeded(icmp_port_t *icmp_port, acc_t *ip_pack, int ip_hdr_len,
			       ip_hdr_t *ip_hdr, acc_t *icmp_pack, int icmp_len,
			       icmp_hdr_t *icmp_hdr);
static void icmp_router_advertisement(icmp_port_t *icmp_port, acc_t *icmp_pack, int icmp_len,
				      icmp_hdr_t *icmp_hdr);
static void icmp_redirect(icmp_port_t *icmp_port, ip_hdr_t *ip_hdr, acc_t *icmp_pack,
			  int icmp_len, icmp_hdr_t *icmp_hdr);
static acc_t *make_repl_ip(ip_hdr_t *ip_hdr, int ip_len);
...
icmp_port_t *icmp_port;
{
	...
	case ICPS_BEGIN:
		...
		{
			DBLOCK(1, printk("unable to open ip_port %d\n",
	...
	case ICPS_IPOPT:
		...
		break;
	default:
		DBLOCK(1, printk("unknown state %d\n",
			icmp_port->icp_state));
		break;
	}
}
...
int for_ioctl;
{
	...
	if (icmp_port->icp_flags & ICPF_WRITE_IP)
	{
		if (!count)
		{
			...
			if (result<0)
			{
				DBLOCK(1, printk("got write error %d\n",
					result));
			}
...
int for_ioctl;
{
	...
	if (icmp_port->icp_flags & ICPF_READ_IP)
	{
		if (!data)
		{
			...
			if (result<0)
			{
				DBLOCK(1, printk("got read error %d\n",
					result));
			}
...
int code;
{
	...
	{
		printk("icmp_snd_time_exceeded: strange port %d\n", port_nr);
		bf_afree(pack);
		return;
	}
...
ipaddr_t gw;
{
	...
	{
		printk("icmp_snd_redirect: strange port %d\n", port_nr);
		bf_afree(pack);
		return;
	}
	pack= icmp_err_pack(pack, &icmp_hdr);
...
int code;
{
	...
	{
		printk("icmp_snd_unreachable: strange port %d\n", port_nr);
		bf_afree(pack);
		return;
	}
	pack= icmp_err_pack(pack, &icmp_hdr);
...
void icmp_snd_mtu(port_nr, pack, mtu)
int port_nr;
acc_t *pack;
u16_t mtu;
{
	...
	{
		printk("icmp_snd_mtu: strange port %d\n", port_nr);
		bf_afree(pack);
		return;
	}
...
acc_t *data;
{
	...
	if (pack_len < ICMP_MIN_HDR_SIZE)
	{
		if (pack_len == 0 && ip_hdr->ih_proto == 0)
		{
			...
			DBLOCK(1, printk("got an incomplete icmp packet\n"));
		bf_afree(data);
		return;
	}
	...
	case ICMP_TYPE_SRC_QUENCH:
		...
		DBLOCK(2, printk("ignoring SRC QUENCH ICMP.\n"));
		break;
	...
	case ICMP_TYPE_TIME_EXCEEDED:
		...
		break;
	default:
		DBLOCK(1, printk("got an unknown icmp (%d) from ",
			icmp_hdr->ih_type); 
			writeIpAddr(ip_hdr->ih_src); printk("\n"));
		break;
	}
	bf_afree(data);
	bf_afree(icmp_data);
...
icmp_hdr_t *icmp_hdr;
{
	...
	{
		DBLOCK(1,
		printk("got an icmp echo request with unknown code (%d)\n",
			icmp_hdr->ih_code));
		bf_afree(ip_data);
		bf_afree(icmp_data);
		return;
	...
	{
		DBLOCK(1, printk("got an incomplete icmp echo request\n"));
		bf_afree(ip_data);
		bf_afree(icmp_data);
		return;
	}
	...
	DBLOCK(2,
	printk("ih_chksum= 0x%x, ih_type= 0x%x, repl->ih_type= 0x%x\n",
	...
	DBLOCK(2, printk("sending chksum 0x%x\n", repl_icmp_hdr->ih_chksum));
...
acc_t *reply_ip_hdr;
{
	...
	printk("icmp[%d]: dropping ICMP packet #%d to ",
	...
	case ICMP_TYPE_PARAM_PROBLEM:
		...
		icmp_hdr= (icmp_hdr_t *)(ptr2acc_data(pack)+hdrlen);
		printk("\tinfo %08x, original dst ",
			ntohs(icmp_hdr->ih_hun.ihh_unused));
		writeIpAddr(ip_hdr->ih_dst);
		printk(", proto %d, length %u\n",
...
ev_arg_t ev_arg;
{
	...
	while (icmp_port->icp_head_queue != NULL)
	{
		...
		if (result != -EWOULDBLOCK)
		{
			if (result == 0)
				continue;
			DBLOCK(1, printk("icmp_write: error %d\n", result););
			continue;
		}
...
icmp_hdr_t *icmp_hdr;
{
	...
	{
		DBLOCK(1, printk("dest unrch with wrong size\n"));
		return;
	}
	...
	{
		DBLOCK(1, printk("dest unrch based on wrong packet\n"));
	...
	case ICMP_FRAGM_AND_DF:
		...
		DBLOCK(1, printk("icmp_dst_unreach: got mtu icmp from ");
			writeIpAddr(ip_hdr->ih_src);
			printk("; original destination: ");
	...
	default:
		DBLOCK(1, printk("icmp_dst_unreach: got strange code %d from ",
			icmp_hdr->ih_code);
			writeIpAddr(ip_hdr->ih_src);
			printk("; original destination: ");
			writeIpAddr(old_ip_hdr->ih_dst);
...
icmp_hdr_t *icmp_hdr;
{
	...
	if (icmp_len < 8 + IP_MIN_HDR_SIZE)
	{
		DBLOCK(1, printk("time exceeded with wrong size\n"));
		return;
	}
	old_ip_pack= bf_cut (icmp_pack, 8, icmp_len-8);
	old_ip_pack= bf_packIffLess(old_ip_pack, IP_MIN_HDR_SIZE);
	...
	if (old_ip_hdr->ih_src != ip_hdr->ih_dst)
	{
		DBLOCK(1, printk("time exceeded based on wrong packet\n"));
		bf_afree(old_ip_pack);
		return;
	}
	...
	case ICMP_FRAG_REASSEM:
		...
		break;
	default:
		DBLOCK(1, printk("got strange code: %d\n",
			icmp_hdr->ih_code));
		break;
	}
	bf_afree(old_ip_pack);
...
icmp_hdr_t *icmp_hdr;
{
	...
	{
		DBLOCK(1,
		printk("router advertisement with wrong size (%d)\n",
			icmp_len));
		return;
	}
	if (icmp_hdr->ih_code != 0)
	{
		DBLOCK(1,
		printk("router advertisement with wrong code (%d)\n", 
			icmp_hdr->ih_code));
		return;
	}
	...
	{
		DBLOCK(1,
			printk("router advertisement with wrong size\n"); 
			printk(
...
icmp_hdr_t *icmp_hdr;
{
	...
	if (icmp_len < 8 + IP_MIN_HDR_SIZE)
	{
		DBLOCK(1, printk("redirect with wrong size\n"));
		return;
	}
	old_ip_pack= bf_cut (icmp_pack, 8, icmp_len-8);
	old_ip_pack= bf_packIffLess(old_ip_pack, IP_MIN_HDR_SIZE);
...
icmp_hdr_t **icmp_hdr_pp;
{
	...
	{
		printk("icmp_err_pack: invalid source address: ");
		writeIpAddr(dest);
		printk("\n");
		bf_afree(pack);
		return NULL;
	...
	{
		printk("icmp_err_pack: wrong packet size:\n");
		printk("\thdrlen= %d, ih_length= %d, bufsize= %d\n",
