...
static int eth_checkopt(eth_fd_t *eth_fd);
static void hash_fd(eth_fd_t *eth_fd);
static void unhash_fd(eth_fd_t *eth_fd);
static void eth_buffree(int priority);
#ifdef BUF_CONSISTENCY_CHECK
static void eth_bufcheck(void);
#endif
static int eth_sel_read(eth_fd_t *);
static void packet2user(eth_fd_t *fd, acc_t *pack, time_t exp_time);
static void reply_thr_get(eth_fd_t *eth_fd, size_t result, int for_ioctl);
static void reply_thr_put(eth_fd_t *eth_fd, size_t result, int for_ioctl);
static void do_rec_conf(eth_port_t *eth_port);
static u32_t compute_rec_conf(eth_port_t *eth_port);
static acc_t *insert_vlan_hdr(eth_port_t *eth_port, acc_t *pack);
