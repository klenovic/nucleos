...
static int freesp_inode(struct ext2_inode *rip, off_t st, off_t end);
static int remove_dir(struct ext2_inode *rldirp, struct ext2_inode *rip, char dir_name[NAME_MAX + 1]);
static int unlink_file(struct ext2_inode *dirp, struct ext2_inode *rip, char file_name[NAME_MAX + 1]);
static off_t nextblock(off_t pos, int blocksize);
static void zeroblock_half(struct ext2_inode *i, off_t p, int l);
static void zeroblock_range(struct ext2_inode *i, off_t p, off_t h);
...
/* Perform the link(name1, name2) system call. */
  ...
  struct ext2_inode *ip, *rip;
  register int r;
  char string[NAME_MAX + 1];
  struct ext2_inode *new_ip;
  phys_bytes len;
  ...
  r = 0;
  if (rip->i_links_count >= USHORT_MAX)
	  r = -EMLINK;
  ...
  if (ip->i_links_count == 0) {	/* Dir does not actually exist */
  	put_inode(rip);
	put_inode(ip);
	return(-ENOENT);
  }
 ...
 */
  ...
  if(fs_m_in.m_type == REQ_UNLINK) {
	  ...
	  if (r == 0) r = unlink_file(rldirp, rip, string);
  } else {
...
struct ext2_inode *rldirp;		 	/* parent directory */
struct ext2_inode *rip;			/* directory to be removed */
char dir_name[NAME_MAX + 1];	/* name of directory to be removed */
{
  ...
  if ((r = search_dir(rip, "", NULL, IS_EMPTY, IGN_PERM, 0)) != 0)
	return r;
  ...
  /* Actually try to unlink the file; fails if parent is mode 0 etc. */
  if ((r = unlink_file(rldirp, rip, dir_name)) != 0) return r;
...
struct ext2_inode *dirp;		/* parent directory of file */
struct ext2_inode *rip;		/* inode of file, may be NULL too. */
...
/* Unlink 'file_name'; rip must be the inode of 'file_name' or NULL. */
  ...
  if (rip == NULL) {
	...
	if (err_code == 0) rip = get_inode(dirp->i_dev, (int) numb);
	if (err_code != 0 || rip == NULL) return(err_code);
  } else {
...
/* Perform the rename(name1, name2) system call. */
  struct ext2_inode *old_dirp, *old_ip;	/* ptrs to old dir, file inodes */
  struct ext2_inode *new_dirp, *new_ip;	/* ptrs to new dir, file inodes */
  struct ext2_inode *new_superdirp, *next_new_superdirp;
  ...
  if(r == 0) {
	...
	if(new_ip == NULL) {
		...
		if(odir && (new_dirp->i_links_count >= SHORT_MAX ||
  ...
  if(r == 0) {
	...
	/* if r is 0, the rename will succeed, while there is now an
  ...
  /* If r is 0, the ctime and mtime of old_dirp and new_dirp have been marked
   * for update in search_dir. */

  if(r == 0 && odir && !same_pdir) {
	/* Update the .. entry in the directory (still points to old_dirp).*/
	numb = new_dirp->i_num;
	(void) unlink_file(old_ip, NULL, dot2);
	if(search_dir(old_ip, dot2, &numb, ENTER, IGN_PERM, I_DIRECTORY) == 0) {
		/* New link created. */
...
register struct ext2_inode *rip;	/* pointer to inode to be truncated */
 ...
 */
  ...
  if (newsize < rip->i_size) {
	if ((r = freesp_inode(rip, newsize, rip->i_size)) != 0)
		return(r);
  }
...
register struct ext2_inode *rip;	/* pointer to inode to be partly freed */
 ...
 */
  ...
  if (start/block_size == (end-1)/block_size && (zero_last || zero_first)) {
	...
	for (p = nextblock(start, block_size)/block_size; p < e; p++) {
		if ((r = write_map(rip, p*block_size, NO_BLOCK, WMAP_FREE)) != 0)
			return(r);
	}
  }
 ...
 */
  ...
  if ( (bp = get_block(rip->i_dev, b, NORMAL)) == NULL)
	panic("EXT2","zeroblock_range: no block",NO_NUM);
  offset = pos % rip->i_sp->s_block_size;
  if (offset + len > rip->i_sp->s_block_size)
	panic("EXT2","zeroblock_range: len too long", len);
