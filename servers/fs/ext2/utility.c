 ...
 */
  ...
  if ( (k=getuptime2(&uptime, &boottime)) != 0)
		panic("EXT2","clock_time: getuptme2 failed: %d", k);
		      ...
		      unsigned int maxlen)
{
  if(len < maxlen && str[len-1] != '\0') {
	printk("ext2 %s:%d string (length %d, maxlen %d) not null-terminated\n",
		file, line, len, maxlen);
  }
}

#define MYASSERT(c) if(!(c)) { printk("ext2:%s:%d: sanity check: %s failed\n", \
  file, line, #c); panic("EXT2","sanity check " #c " failed: %d", __LINE__); }
 ...
 *				setbyte   				     *
 *===========================================================================*/
u32 setbyte(bitchunk_t *bitmap, u32 max_bits, unsigned int word)
{
