...
int fs_mkdir()
{
  ...
  if (r1 == 0 && r2 == 0) {
	  ...
	  if (search_dir(ldirp, lastc, NULL, DELETE, IGN_PERM, 0) != 0)
		  panic("EXT2","Dir disappeared ", rip->i_num);
...
int fs_slink()
{
  ...
  if( (r = err_code) == 0) {
	...
	if(r != 0) {
		sip->i_links_count = 0;
		if (search_dir(ldirp, string, NULL, DELETE, IGN_PERM, 0) != 0)
			panic("EXT2","Symbolic link vanished",NO_NUM);
	}
  }
 ...
 */
  ...
  if (S_ISDIR(bits) && (ldirp->i_links_count >= USHORT_MAX ||
