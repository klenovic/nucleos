  ...
  *completed = 0;
  ...
  if (block_spec) {
	...
	if (ex64hi(position) != 0)
		panic("EXT2","rw_chunk: position too high",NO_NUM);
  ...
  if (bp == NULL)
	panic("EXT2","bp not valid in rw_chunk, this can't happen",NO_NUM);
...
# define BLOCKS_MINIMUM		(nr_bufs < 50 ? 18 : 32)
  ...
  if(readqsize != nr_bufs) {
	...
	if(!(read_q = malloc(sizeof(read_q[0])*nr_bufs)))
		panic("EXT2","couldn't allocate read_q",NO_NUM);
	readqsize = nr_bufs;
  }
...
int fs_getdents(void)
{
  ...
  for (; block_pos < rip->i_size; block_pos += block_size) {
	...
	if (bp == NO_BLOCK)
		panic("EXT2","get_block returned NO_BLOCK",NO_NUM);
	assert(bp != NULL);
