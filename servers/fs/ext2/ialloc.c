...
static u32 alloc_inode_bit(struct super_block *sp, struct inode *parent, int is_dir);
static void free_inode_bit(struct super_block *sp, u32 bit_returned, int is_dir);
...
/* Allocate a free inode on parent's dev, and return a pointer to it. */
  ...
  if ((rip = get_inode(NO_DEV, inumb)) == NULL) {
	...
	rip->i_links_count = 0; /* initial no links */
...
int is_dir;			/* inode will be a directory if it is TRUE */
{
  ...
  if (sp->s_rd_only)
	panic("EXT","can't alloc inode on read-only filesys.",NO_NUM);
  ...
  if (inumber > sp->s_inodes_count) {
	panic("EXT2","ext2: allocator returned inum greater, than\
		    total number of inodes.\n",NO_NUM);
  }

  if (inumber < EXT2_FIRST_INO(sp)) {
	panic("EXT2","ext2: allocator tryed to use reserved inode.\n",NO_NUM);
  }
 ...
 *                          free_inode_bit                                   *
 *===========================================================================*/
static void free_inode_bit(struct super_block *sp, u32 bit_returned,
                           int is_dir)
{
 /* Return an inode by turning off its bitmap bit. */
  ...
  if (bit_returned > sp->s_inodes_count ||
	...
	panic("EXT2","trying to free inode %d beyond inodes scope.", bit_returned);
  ...
  if (unsetbit(bp->b_bitmap, bit))
	panic("EXT2","Tried to free unused inode", bit_returned);
...
static int find_group_dir(struct super_block *sp, struct inode *parent)
{
  ...
  for (group = 0; group < sp->s_groups_count; ++group) {
	gd = get_group_desc(group);
	if (gd == NULL)
		panic("EXT2","can't get group_desc to alloc inode",NO_NUM);
