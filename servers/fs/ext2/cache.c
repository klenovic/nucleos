...
int vm_yield_block_get_block(u64 yieldid, u64 getid, void *mem, vir_bytes len)
{
	return -ENOSYS;
}
/* __Fakes */
 ...
 */
  ...
  if ((bp = front) == NULL) panic("EXT2","all buffers in use", nr_bufs);

  if(bp->b_bytes < fs_block_size) {
	ASSERT(!bp->bp);
	ASSERT(bp->b_bytes == 0);
	if(!(bp->bp = alloc_contig( (size_t) fs_block_size, 0, NULL))) {
		printk("ext2: couldn't allocate a new block.\n");
		for(bp = front;
			bp && bp->b_bytes < fs_block_size; bp = bp->b_next)
			;
		if(!bp) {
			panic("EXT2","no buffer available",NO_NUM);
		}
	} else {
  ...
  if(only_search == PREFETCH) {
	...
	panic("EXT2","unexpected only_search value: %d", only_search);
 ...
 */
  ...
  if ( (dev = bp->b_dev) != NO_DEV) {
	...
	if (r < 0) {
		printk("Ext2(%d) I/O error on device %d/%d, block %lu\n",
...
/* Flush all dirty blocks for one device. */
  ...
  if(dirtylistsize != nr_bufs) {
	...
	if(!(dirty = malloc(sizeof(dirty[0])*nr_bufs)))
		panic("EXT2", "couldn't allocate dirty buf list", NO_NUM);
...
/* Read or write scattered data from a device. */
  ...
  /* Set up I/O vector and do I/O.  The result of dev_io is 0 if everything
...
void set_blocksize(unsigned int blocksize)
{
  ...
  for (bp = &buf[0]; bp < &buf[nr_bufs]; bp++)
	if(bp->b_count != 0) panic("EXT2", "change blocksize with buffer in use",NO_NUM);

  for (rip = &inode[0]; rip < &inode[NR_INODES]; rip++)
	if (rip->i_count > 0) panic("EXT2", "change blocksize with inode in use",NO_NUM);
...
/* Initialize the buffer pool. */
  ...
  if(!(buf = calloc(sizeof(buf[0]), new_nr_bufs)))
	panic("EXT2", "couldn't allocate buf list (%d)", new_nr_bufs);

  if(buf_hash)
	free(buf_hash);
  if(!(buf_hash = calloc(sizeof(buf_hash[0]), new_nr_bufs)))
	panic("EXT2", "couldn't allocate buf hash list (%d)", new_nr_bufs);
