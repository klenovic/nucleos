 ...
 */
  ...
  while (TRUE) {
      ...
      if (is_notify(m.m_type)) {
          switch (who_p) {
            case CLOCK:
	      do_period(&m);			/* check modules status */
              continue;
 ...
 *	                    exec_image_copy                                  *
 *===========================================================================*/
static void exec_image_copy(boot_proc_idx, ip, rp)
int boot_proc_idx;
...
/* Copy the executable image of the given boot process. */
  ...
  if(boot_image_buffer+boot_image_buffer_size - boot_image_ptr < sizeof(header)
      || (s = sys_getaoutheader(&header, boot_proc_idx)) != 0) {
 ...
 *			      fill_call_mask                                 *
 *===========================================================================*/
static void fill_call_mask(calls, tot_nr_calls, call_mask, call_base)
...
/* Initialize the reincarnation server. */
  ...
  for(i=0;i<NR_BOOT_PROCS;i++) {
      ...
      if(boot_image_sys->flags & SF_USE_COPY) {
          if((s = sys_getaoutheader(&header, i)) != 0) {
              panic("RS", "unable to get copy of a.out header", s);
          }
  ...
  for (i=0; boot_image_priv_table[i].endpoint != NULL_BOOT_NR; i++) {
      ...
      if ((s = sys_getpriv(&(rp->r_priv), ip->endpoint)) != 0) {
  ...
  for (i=0; boot_image_priv_table[i].endpoint != NULL_BOOT_NR; i++) {
      ...
      if ((s = sys_privctl(ip->endpoint, SYS_PRIV_ALLOW, NULL)) != 0) {
          panic("RS", "unable to initialize privileges", s);
      }
  }

  /* - Step 3: all the system modules in the boot image are now running.
  ...
  if (0 != (s=sys_setalarm(RS_DELTA_T, 0)))
...
kipc_msg_t *m_in;				/* pointer to message */
{
    ...
    if ((s=kipc_module_call(KIPC_RECEIVE, 0, ENDPT_ANY, m_in)) != 0) 	/* wait for message */
