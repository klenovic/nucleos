 ...
 */
   ...
   */
  if ((r = sys_stop(proc_nr_e)) != 0)		/* stop the process */
 ...
 */
  ...
  pidarg  = (call_nr == __NR_wait || call_nr == __NNR_wait ? -1 : m_in.pid);	   /* 1st param of waitpid */
  options = (call_nr == __NR_wait || call_nr == __NNR_wait ?  0 : m_in.sig_nr);  /* 3rd param of waitpid */
