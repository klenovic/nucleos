...
struct vnode {
  endpoint_t v_fs_e;            /* FS_PROC_NR process' endpoint number */
  ...
  int v_fs_count;		/* # reference at the underlying FS_PROC_NR */
  int v_ref_check;		/* for consistency checks */
  char v_pipe;			/* set to I_PIPE if pipe */
  off_t v_pipe_rd_pos;
  off_t v_pipe_wr_pos;
  endpoint_t v_bfs_e;		/* endpoint number for the FS_PROC_NR proces in case
